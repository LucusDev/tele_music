...
class MyState extends State {
  ...
  void applySuccess() async {
    String res =
        await PlanPresenter().addPlanCoupon(codeController.text, token);
  ...
  Widget build(BuildContext context) {
    return SafeArea(
      ...
      child: ListView(children: [
        ...
        if (modelSettings.payment_gateways.stripe.stripe_client_id.isEmpty &&
            modelSettings
                .payment_gateways.paystack.paystack_public_key.isEmpty &&
            modelSettings.payment_gateways.razorpay.razorpay_key.isEmpty &&
            modelSettings.payment_gateways.paypal.paypal_client_id.isEmpty)
          Text(
            '  Currently payment not accepted by admin !',
            style: TextStyle(
                fontFamily: 'Nunito-Bold',
                fontSize: 19.0,
                color: (sharedPreThemeData.themeImageBack.isEmpty)
        ...
        Row(
          ...
          children: [
            if (hasData)
              if (modelSettings
                  .payment_gateways.paypal.paypal_client_id.isNotEmpty)
                Container(
            ...
            if (hasData)
              if (modelSettings
                  .payment_gateways.stripe.stripe_client_id.isNotEmpty)
                Container(
        ...
        Row(mainAxisAlignment: MainAxisAlignment.spaceEvenly, children: [
          if (hasData)
            if (modelSettings
                .payment_gateways.paystack.paystack_public_key.isNotEmpty)
              Container(
          ...
          if (hasData)
            if (modelSettings.payment_gateways.razorpay.razorpay_key.isNotEmpty)
              Container(
