...
class purchase_state extends State {
  ...
  Future<void> apiCall() async {
    String response =
        await PurchaseHistoryPresenter().purchaseHistoryInfo(token);
    if (response.isEmpty) {
      isLoading = false;
      setState(() {});
    } else {
                        ...
                        SliverToBoxAdapter(
                            child: (audioPurchaseHistory.length == 0)
                                ? Container(
                                    ...
                                    alignment: Alignment.center,
                                    child: Text("History not found of audio !",
                                        style: TextStyle(
                                            fontSize: 15,
                                    ...
                                    itemBuilder: (context, index) {
                                      ...
                                      try {
                                        final Map<String, dynamic> parsed =
                                            json.decode(audioData);
                                        List<dynamic> list =
                                            json.decode(payment_data);
                        ...
                        SliverToBoxAdapter(
                            child: (planPurchaseHistory.length == 0)
                                ? Container(
                                    margin: EdgeInsets.fromLTRB(2, 12, 2, 29),
                                    alignment: Alignment.center,
                                    child: Text("History not found of plan !",
                                        style: TextStyle(
                                            fontSize: 15,
                                            color: appColors().colorHint)),
                                  )
                                : ListView.builder(
                                    ...
                                    itemBuilder: (context, index) {
                                      String planData =
                                          planPurchaseHistory[index].plan_data;
                                      String payment_data =
                                          planPurchaseHistory[index]
                                              .payment_data;

                                      final Map<String, dynamic> parsed =
                                          json.decode(planData);
                                      List<dynamic> list =
                                          json.decode(payment_data);
                                      Fact fact = Fact.fromJson(list[0]);
