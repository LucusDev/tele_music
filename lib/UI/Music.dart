...
class MainScreen extends State<Music> {
  ...
  void downloadAfterPayment() {
    downloadFile(
        currentData.extras?.entries.first.value,
        ...
        if (audioPathMain.contains('Local')) {
          ...
          if (type.isEmpty) {
            ...
            for (int x = 0; x < listCopy.length; x++) {
              ...
              String audiourl = listCopy[x].audio;
  ...
  Future<void> getByCat() async {
    ...
    for (int x = 0; x < mList.data.length; x++) {
      ...
      String audiourl = mList.data[x].audio;
      String imageUrl = "";
      ...
      listDataa.add(MediaItem(
          ...
          duration: Duration(
              minutes: int.parse(parts[0]), seconds: int.parse(parts[1])),
          artUri: Uri.parse(imageUrl),
          extras: {'audio_id': mList.data[x].id.toString()}));
    }
    _mediaLibrary = MediaLibrary();
  ...
  Future<void> downloadFile(String audioId, String duration, String name,
    ...
    try {
      var apiRespon = await dio.download(
          '$artUri', "${dir.path}/$audioId" + "img.jpg",
          onReceiveProgress: (rec, total) {});
      apiRespon.data;
    } catch (e) {}
    setState(() {
                              ...
                              children: [
                                ...
                                StreamBuilder<PositionData>(
                                  ...
                                  builder: (context, snapshot) {
                                    ...
                                    min = 0.0;
                                    maxi = positionData.duration.inMilliseconds
                                        .toDouble();

                                    if (positionData.position.inMilliseconds
                                            .toDouble() <=
                                        maxi) {
                                      valueHolder = positionData
                          ...
                          children: [
                            ...
                            Container(
                                ...
                                child: InkResponse(
                                  onTap: () {
                                    print(
                                        "TESTT --${currentData.extras?.entries.first.value}");
                                    isOpen = true;
                                    if (!audioPathMain.contains("Local")) {
                                      if (currentData.artUri
                                          .toString()
                                          .contains("file://")) {
                                        Fluttertoast.showToast(
                                            msg:
                                                'Not able to perform action in downloaded songs',
                                            toastLength: Toast.LENGTH_SHORT,
                                            timeInSecForIosWeb: 1,
                                            backgroundColor: Colors.grey,
                                            textColor:
                                      ...
                                      Fluttertoast.showToast(
                                          msg:
                                              'You are offline! Not able to perform action here',
                                          toastLength: Toast.LENGTH_SHORT,
                                          timeInSecForIosWeb: 1,
                                          backgroundColor: Colors.grey,
                                          textColor:
