...
class ModelPlanList {
  ...
  factory ModelPlanList.fromJson(Map<String, dynamic> json) {
    List<DataPlan> d =
        List<DataPlan>.from(json["data"].map((x) => DataPlan.fromJson(x)));
    return ModelPlanList(json['status'], json['msg'], d);
  }
}
...
class DataPlan {
  ...
  factory DataPlan.fromJson(Map<String, dynamic> json) {
    List<SubData> d =
        List<SubData>.from(json["all_plans"].map((x) => SubData.fromJson(x)));

    return DataPlan(CurrentPlanData.fromJson(json['current_plan'] ?? {}), d);
  }
}
