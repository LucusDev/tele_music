...
class LoginDataPresenter {
  ...
  Future<String> getUser(BuildContext context, String buildNumber, String email,
      String pass) async {
    var formData = FormData.fromMap(
        {AppConstant.email: email, AppConstant.password: pass});

    try {
      Response<String> response = await _dio
          .post(AppConstant.BaseUrl + AppConstant.API_LOGIN, data: formData);
      ...
      if (response.statusCode == 200) {
        ...
        if (parsed['status'].toString().contains('true')) {
          ...
          sharePrefs.setThemeData(jsonEncode(ModelTheme('', '', 'Default theme',
              '0xFFb5bada', 'assets/images/default_screen.jpg', 'free')));
          return "1";
        } else {
